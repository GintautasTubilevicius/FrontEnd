        ...
        <>
            {!gin.drinks ? (<Loading />) : (
                <section className="py-32 flex flex-col items-center">
                    <h1 className="heading text-center pb-16 ">Gin Coctails (overall {gin.drinks.length} )</h1>
                    <div className="container flex items-center flex-wrap pt-4 pb-12">

                        {
                            gin.drinks.map(({ idDrink, strDrink, strDrinkThumb }) =>
                            (
                                <div key={idDrink} className="w-full md:w-1/3 xl:w-1/4 p-6 flex flex-col">
                                <Link to={`/coctails/${idDrink}`}>
                                    <img className="hover:grow hover:shadow-lg" src={strDrinkThumb} alt="" />
                                    <div className="pt-3 flex items-center justify-between">
                                        <p className="text-white ">{strDrink}</p>
