...
export default function Books() {
    ...
    const EditButton = Button(showEditDialog, 'Redaguoti');
    const DeleteButton = Button(deleteElement, 'Trinti');
...
const Row = (element, i, buttons) => (
    <tr key={i}>
        <td>{element.title}</td>
        <td>{element.name}</td>
        <td className="center">{element.release_year}</td>
        <td className="center">{buttons[0](element)}</td>
        <td className="center">{buttons[1](element)}</td>
    </tr>
)
            ...
            <form method="post" action="/api/books/edit" onSubmit={submit}>
                ...
                <div>
                    ...
                    <div key={'genre_'+ entry.genre_id}>
                        <label htmlFor="genre_input">Žanras: </label>
                        <select id="genre_input" name="genre">
                            {genres.map((genre, i) => (
                                <option key={i} value={genre.genre_id} defaultValue={genre.genre_id === entry.genre_id}>{genre.name}</option>
                            ))}
                        </select>
                    </div>
